...
class BLSTM_CRF(object):
    def __init__(self, embedded_chars, hidden_unit, cell_type,num_layers, droupout_rate,
        ...
        """
        self.hidden_unit = hidden_unit  #128
    ...
    def add_blstm_crf_layer(self):
        ...
        #crf
        loss, trans = self.crf_layer(logits) #（64,128,17）
    ...
    def project_bilstm_layer(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope("project" if not name else name):
            with tf.variable_scope("hidden"):
                W = tf.get_variable("W", shape=[self.hidden_unit, self.hidden_unit],
                                    dtype=tf.float32, initializer=self.initializers.xavier_initializer())

                b = tf.get_variable("b", shape=[self.hidden_unit], dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
                output = tf.reshape(lstm_outputs, shape=[-1, self.hidden_unit])
                print('output shape:',output.get_shape())  #24576*256
                hidden = tf.tanh(tf.nn.xw_plus_b(W, output, b))
                print('hedden shape:',hidden.get_shape()) #8192*256
            ...
            with tf.variable_scope("logits"):
                ...
                print('pred shape:',pred.get_shape()) #[8192,17]
            return tf.reshape(pred, [-1, self.seq_length, self.num_labels]) #[64,128,17]
    ...
    def crf_layer(self, logits):
        ...
        with tf.variable_scope("crf_loss"):
            trans = tf.get_variable(
                ...
                initializer=self.initializers.xavier_initializer()) #[17,17] 状态转移矩阵
            log_likelihood, trans = tf.contrib.crf.crf_log_likelihood(
                inputs=logits, #[64,128,17]
                tag_indices=self.labels,
                transition_params=trans, #[17,17]
                sequence_lengths=self.lengths) #[64,]
