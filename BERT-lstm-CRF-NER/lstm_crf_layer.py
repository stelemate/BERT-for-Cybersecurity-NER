...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self):
        ...
        #blstm
        lstm_output = self.lstm_layer(self.embedded_chars)
        #project
        logits = self.project_lstm_layer(lstm_output)
        #crf
    ...
    def lstm_layer(self, embedding_chars):
        """
                
        :return: 
        """
        with tf.variable_scope('rnn_layer'):
            lstmCell = tf.contrib.rnn.BasicLSTMCell(self.hidden_unit)
            outputs, _ = tf.nn.dynamic_rnn(lstmCell,embedding_chars,
                                                         dtype=tf.float32)
            outputs = tf.concat(outputs, axis=2)
        return outputs

    def project_lstm_layer(self, lstm_outputs, name=None):
        """
        ...
        with tf.variable_scope("project" if not name else name):
            ...
            with tf.variable_scope("logits"):
                ...
                output = tf.reshape(lstm_outputs,
                                    shape=[-1, self.hidden_unit])  # [batch_size, embedding_dims]
